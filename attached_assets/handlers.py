...
from user_data import (
    ...
    get_shop_item, purchase_buff, get_active_buffs_info, SHOP_ITEMS
)
...
def register_handlers(bot):
    ...
    def shop_callback(call):
        ...
        if call.data == "shop_prev":
            ...
            current_pos = (current_pos - 1) % len(SHOP_ITEMS)
...
def profile_command(message):
    ...
    profile_message = (
        ...
        f"📦 Коробок разнесено: *{user_data['total_deliveries']}* шт.\n"
        f"💡 Опыт разносчика: *{user_data['experience']}* ✨\n"
        f"💰 Баланс: *{user_data.get('total_earnings', 0)}* рублей 💵"
        f"{buffs_message}\n\n"
...
def show_shop_item(chat_id, user_id, item_index, message_id=None):
    ...
    caption = (
        ...
        f"💵 Ваш баланс: *{user_data.get('total_earnings', 0)}* рублей\n\n"
        f"{item_index + 1}/{len(SHOP_ITEMS)}"
    )
    
    # Try to open the image
    image_path = item["image_path"]
    if not os.path.exists(image_path):
        # If image doesn't exist, use a fallback
        image_path = create_delivery_image()
        logger.warning(f"Shop image not found: {image_path}, using fallback")
